  ...
  return (
    <div className="flex justify-center h-screen">
      ...
      <div className="absolute top-4 right-4 md:hidden">
        <button onClick={() => setIsToolsPanelOpen(true)} className="text-accent">
          <Menu size={24} />
        </button>
      </div>
      ...
      {isToolsPanelOpen && (
        <div className="fixed inset-0 z-50 flex justify-end bg-black bg-opacity-30">
          <div className="w-full bg-surface-2 text-fg h-full p-4">
            <button className="mb-4 text-accent" onClick={() => setIsToolsPanelOpen(false)}>
              <X size={24} />
            </button>
            <ToolsPanel />
          </div>
